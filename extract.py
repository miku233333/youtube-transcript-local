...
class URLValidationError(TranscriptError):
    ...
    def __init__(self, message: str = "Invalid YouTube URL", url: Optional[str] = None):
        super().__init__(
            message=message, error_code="URL_VALIDATION_FAILED", details={"url": url}
        )
...
class YTDLLError(TranscriptError):
    ...
    def __init__(
        self, message: str, stderr: Optional[str] = None, returncode: int = -1
    ):
        super().__init__(
            message=message,
...
class YouTubeTranscriptExtractor:
    ...
    def _extract_subtitles(
        self, url: str, video_id: str, lang: str, auto_generate: bool = False
    ) -> Optional[Path]:
        """
        提取字幕
    ...
    def extract(
        self, url: str, lang: str = "en", auto_generate: bool = True
    ) -> Dict[str, Any]:
        """
        提取字幕（主接口）
...
示例:
  ...
  %(prog)s -u VIDEO_ID -o ./output --log-file ./logs/extract.log
    ...
    parser.add_argument(
        "-l", "--lang", default="zh-Hans", help="語言代碼 (default: zh-Hans)"
    )
    ...
    try:
        extractor = YouTubeTranscriptExtractor(output_dir=args.output, logger=logger)
